...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({
      address: p.addressLine1 || street, city: p.city || city, state: p.state || state,
      zip: p.zipCode || zip, county: p.county || '',
      beds: p.bedrooms ?? null, baths: p.bathrooms ?? null, sqft: p.squareFootage ?? null,
      year_built: p.yearBuilt ?? null, property_type: p.propertyType ?? null,
      stories: p.stories ?? null, lot_size: p.lotSize ?? null,
      apn: p.apn ?? null, owner_name: p.ownerName ?? null, estimated_value: p.price ?? null,
    });
